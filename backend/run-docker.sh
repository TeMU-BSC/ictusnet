...
# https://github.com/TeMU-BSC/ictusnet-ctakes/blob/master/run-docker.sh
...
if [ -z "$1" ] || [ -z "$2" ]
  ...
  docker run --rm -v $INPUT_DIR:/ICTUSnet/data/TXT -v $OUTPUT_DIR:$OUTPUT_DIR/ANN_FINAL/ bsctemu/ictusnet process.sh $OUTPUT_DIR #without -it
fi
